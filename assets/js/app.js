...
let newCountries = countries.map(ele => {
    // cl(ele);
    if(ele.capital == undefined){
        return ele.capital = "NA";
    }else{
        return ele;
    }
})
...
const onkeyUpHandler = (e) => {
    ...
    // cl(values);
    let newArr = countries.filter(ele => {
        // cl(ele);
        return ele.name.toLowerCase().includes(values);
    })
    templating(newArr);
}
...
// let onClick1 = (event) =>{
  // event.target.classList.toggle("fa-arrow-down-long")   
 // console.log(event.target);
 // let newCountries = countries.sort().reverse()
...
// }
  ...
  // icons.forEach(ele => {
    ...
    // ele.addEventListener("click", onClick1)
  // })
...
function onClickHandler(e){
    ...
    if(nameFlag == true){
		...
		// e.target.classList.toggle("fa-arrow-down-long");
        templating(countries);
        nameFlag = true;
    }
}
...
search.addEventListener("keyup", onkeyUpHandler);

nameBtn.addEventListener("click", onClickHandler);

capitalBtn.addEventListener("click", onClick);

populationBtn.addEventListener("click", onBtnClick)